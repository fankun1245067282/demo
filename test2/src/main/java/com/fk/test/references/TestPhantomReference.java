...
public class TestPhantomReference {
	...
	public static void checkElements(Set<PhantomReference<Grocery>> pa) {
		int num = 0;
		for(int i=0; i<5; i++) {
			try {
			...
			for (Reference<? extends Grocery> inq : pa) {
				if (inq != null) {
					try {
						num++;
						Field field = inq.getClass().getSuperclass().getDeclaredField("referent");
						field.setAccessible(true);// 可以访问私有，final属性，修改它
						Object obj = field.get(inq);
						System.out.println("Element num: " + (num<10?("0"+num):(num)) + "  value: " + inq.get() + "  value2: " + obj + "==" + inq);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	 ...
	 * 当垃圾收集后，进入到pending队列，不管reference.queue是否为null，为null的进入不了队列 断点发现：
	 * 当为weakReference时，pending对象的referent为空， 当为SoftReference时，pending对象的referent为空，
