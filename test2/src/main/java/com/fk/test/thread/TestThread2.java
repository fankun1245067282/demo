...
public class TestThread2 {
	...
	public final synchronized void testwait() throws InterruptedException {
		System.out.println("Thread:"+Thread.currentThread().getName()+" this is t1 method doing 1 "+sdf.format(new Date()));
		wait(5000);
		Thread.sleep(2000);
		System.out.println("Thread:"+Thread.currentThread().getName()+" this is t1 method doing 2 "+sdf.format(new Date()));
	}
...
//		t1.interrupt();
	...
	public final synchronized void testnotifyall() throws InterruptedException {
		System.out.println("Thread:"+Thread.currentThread().getName()+" this is t1 method doing 1 "+sdf.format(new Date()));
		this.notifyAll();
		System.out.println("Thread:"+Thread.currentThread().getName()+" this is t1 method doing 2 "+sdf.format(new Date()));
	}
	...
	@Test
	public void testnotifyallmain() throws Exception {
		final TestThread2 testThread2 = new TestThread2();
		Runnable r1 = new Runnable() {
			@Override
			public void run() {
		...
		t1.start();
		t2.start();
		Thread.sleep(300);//睡眠一下，防止t1,t2没有执行wait
		System.out.println(Arrays.toString(t2.getStackTrace()));
		t3.start();
